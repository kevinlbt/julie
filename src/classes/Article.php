    ...
    public static function newArticle () {
        ...
        if (isset($_POST)) {
            ...
            && isset($_FILES['images']) && !empty($_FILES['images']) ) {
                ...
                $imagePath = ("./assets/uploads/" . basename($_FILES["images"]["name"]));
                
                $db = DataBase::getInstance();
            
                //first request, insert article into database
                $query = $db->prepare('INSERT INTO `article` (`title`, `content`, `user_id`, `image_path`) VALUES (:title, :content, :user_id, :image_path);');
                $params = [
                    'title' => $title,
                    'content' => $content,
    ...
    public static function updateArticle ($id) {
        ...
        if (isset($_POST)) {
            ...
            && isset($_FILES['images']) && !empty($_FILES['images']) ) {
                ...
                $query = $db->prepare('UPDATE `article` SET `title`=:title, `content`=:content, `image_path`=:image_path WHERE `id`=:id;');
                $params = [
                    'title' => $title,
                    'content' => $content,
                    'id' => $id[1],
